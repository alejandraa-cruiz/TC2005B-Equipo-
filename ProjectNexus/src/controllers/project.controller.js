...
exports.project = async (req, res) => {
  ...
  try {
    ...
    Epic.fetch_unassigned_epics().then((rows, fieldData) => {
      const Epics = rows[0];
    ...
    Project.fetch_name(requestProject.project_name).
        then(([rows, fieldData]) => {
          ...
          if (rows.length === 0) {
            ...
            newProject.save().then(([rows, fieldData]) => {
              ...
              if (listEpicLinks.length > 0) {
                ...
                listEpicLinks.forEach((epic, index) => {
                  if (requestProject[epic] === 'on') listEpicsToInsert.push(
                      epic);
                });
                                ...
                                then((rows, fieldData) => {
                                  ...
                                  newProjectTeam.save().
                                      then(([rows, fieldData]) => {
                                        ...
                                        res.status(500).
                                            json(
                                                {e: 'Database conection failed'});
                                      });
                                }).
                                catch((e) => {
                                  res.status(500).
                                      json({e: 'Database conection failed'});
                                });
...
exports.getListProjects = async (req, res) => {
  ...
  if (projects.length > 0) {
    ...
    const teamMembers = await ProjectTeam.fetch_number_members_assigned(
        projects);
 ...
 */exports.getListProjectsSearchBar = async (req, res) => {
  ...
  if (query) {
    ...
    if (Projects.length === 0) {
      [Projects] = await ProjectTeam.fetch_projects_unassinged_search_bar(
          query);
    }
 ...
 */exports.deleteProject = async (req, res) => {
  ...
  if (rows.affectedRows > 0) res.status(200).
      json({e: 'Success, project was erased'});
...
exports.modifyProject = async (req, res) => {
  ...
  try {
    ...
    Epic.fetch_modify_epics(req.params.project).then((rows, fieldData) => {
      const Epics = rows[0];
...
exports.modifyProjectPost = async (req, res) => {
  ...
  let insertId = 0;
  if (start_date < end_date) {
    Project.modify_by_id(req.body.project_name, req.body.start_date,
        req.body.end_date, req.params.project);
